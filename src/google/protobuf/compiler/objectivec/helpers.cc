...
std::string DefaultValue(const FieldDescriptor* field) {
  ...
  switch (field->cpp_type()) {
    ...
    case FieldDescriptor::CPPTYPE_STRING: {
      ...
      std::string_view default_string = field->default_value_string();
...
void EmitCommentsString(io::Printer* printer, const GenerationOptions& opts,
  ...
  std::string_view comments = location.leading_comments.empty()
  ...
  for (std::string_view l : raw_lines) {
...
bool IsWKTWithObjCCategory(const Descriptor* descriptor) {
  ...
  const std::string_view full_name = descriptor->full_name();
