...
bool IsUpperCamelCase(std::string_view name) {
  if (name.empty()) {
    return true;
  }
...
bool IsUpperUnderscore(std::string_view name) {
  for (const char c : name) {
...
bool IsLowerUnderscore(std::string_view name) {
  for (const char c : name) {
...
bool IsNumberFollowUnderscore(std::string_view name) {
