...
class FlatAllocatorImpl {
 public:
  ...
  void PlanFieldNames(const std::string& name,
    ...
    std::string_view all_names[] = {name, lowercase_name, camelcase_name,
                                     json_name};
  ...
  FieldNamesResult AllocateFieldNames(const std::string_view name,
...
#undef DEFINE_MEMBERS
  ...
  std::string_view full_name() const {
    switch (type()) {
      ...
      case SUB_PACKAGE:
        return std::string_view(sub_package_file_descriptor()->file->package())
...
class DescriptorBuilder {
 ...
 private:
  ...
  const std::string* AllocateNameStrings(std::string_view scope,
...
const std::string* DescriptorBuilder::AllocateNameStrings(
    const std::string_view scope, const std::string_view proto_name,
    internal::FlatAllocator& alloc) {
  if (scope.empty()) {
